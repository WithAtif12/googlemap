...
class OrderOnTheWay extends StatelessWidget {
    ...
    return Container(
      ...
      //pickup location row with icon text and phone button
          ListTile(
            ...
            title: Text('Pickup Location',style: TextStyle(
          ...
          ListTile(
            ...
            title: Text("Delivery - ${order.customerName}",style: TextStyle(
              fontWeight: FontWeight.w600,fontSize: 16,
            ),),
  ...
  //returns appropriate button widgets based on current delivery status
  //difference button type for mark as destination reached button and same button style for remaining all, only change the icon and color
  Widget _buttonStyle() {
    switch(status) {
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  ...
                  child: Center(
                    child: Text(
                      _getButtonTitle(), style: TextStyle(color: Colors.white,
      ...
      default:
        return CustomButton(title: _getButtonTitle(),
            onPressed: _isButtonEnabled() ? (onButtonPressed?? () {}) : () {},color: _getButtonColor(),);
    }
  }
...
Color _getButtonColor(){
  ...
  //returns where button should be enabled/clickable
  ...
  //returns appropriate icon from pickup location based on status
